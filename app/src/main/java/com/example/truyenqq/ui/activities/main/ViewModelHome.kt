package com.example.truyenqq.ui.activities.main

import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.example.truyenqq.R
import com.example.truyenqq.module.models.*
...
class ViewModelHome : ViewModel() {
    ...
    fun loadSchedule() {
        compo.add(
            apiManager.getSchedule("${cal.get(Calendar.YEAR)}-${cal.get(Calendar.MONTH) + 1}-${cal.get(
                Calendar.DATE)}")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
